...
const SpellingContainer =(props) => {
    const {wordList,wordIndex,setNextWordIndex,setCurrentRep, userInput , currentRep, selectedReps, setSpellInput } = props;
    ...
    return(
        ...
        {wordList.length!==0 && (
            <motion.div  variants={dropInBground} initial="hidden" animate="visible" exit="exit" className="mt-4" >
                <motion.div variants={dropInContainer} initial="hidden" animate="visible" exit="exit" className={`d-flex flex-column align-items-center `}>
                    <div >
                        <RepCounter />
                    </div>
