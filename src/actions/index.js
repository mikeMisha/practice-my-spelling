...
export const createWordList = ({validList,filteredList,isSignedIn,userId,name}) =>{
            ...
            if(lists.every((list)=>list.name!==addedList.name)){
               lists.push(addedList)
                ...
                lists = lists.map((savedList)=>{
                    return (savedList.name === addedList.name) ?{name: addedList.name, list: [...savedList.list, ...addedList.list ] } : savedList
                })
            }


            const response = await axios.put(`https://practice-my-spelling.herokuapp.com/users/${userId}`, {lists})
            console.log(response);
            dispatch({ type:'CREATE_WORDLIST',payload:{ lists:response.data.lists, createdList: lists.find((list)=>list.name===addedList.name) } })
        }else{
            dispatch({
                type:'CREATE_WORDLIST',
                payload:{lists:[ addedList ], createdList:{...addedList, list:[...currentWordList.list,...addedList.list]} }
            })
        }
...
export const deleteFromWordList = (deletedWord) =>{
        ...
        return async (dispatch,getState) => {
            ...
            const isShuffleMode = getState().userAppSettings.isShuffleMode
            const filteredCurrentList = currentWordList.list.filter((word)=>word!==deletedWord)
    
            if (isSignedIn){
                const updatedLists = savedLists.map((savedList)=>{
                    return (savedList.name === currentWordList.name) ?{name: currentWordList.name, list: [...filteredCurrentList ] } : savedList
                })
                const response = await axios.patch(`https://practice-my-spelling.herokuapp.com/users/${userId}`,{lists:updatedLists})
                dispatch({type:'DELETE_FROM_WORDLIST',payload:response.data.lists})
            }
         
            if(shuffledList.length>0){
                var filteredShuffledList = shuffledList.filter((word)=>word!==deletedWord)
                dispatch({type:'SET_SHUFFLE_LIST', payload: filteredShuffledList })
            } 
       
            if(cachedList.list) {
                var filteredCachedList = cachedList.list.filter((word)=>word!==deletedWord)
                dispatch({type:'SET_CACHED_LIST', payload:{name:cachedList.name, list:filteredCachedList} })
            }
            
            const newIndex = isShuffleMode ? filteredShuffledList.indexOf(currentWord) : filteredCurrentList.indexOf(currentWord)
           
             dispatch({type:'SET_CURRENT_INDEX',payload: newIndex===-1 ? 0 : newIndex  })
             
            isShuffleMode 
            ? dispatch({type:'SET_CURRENT_LIST', payload:{name:cachedList.name,list:filteredShuffledList}})
            : dispatch({type:'SET_CURRENT_LIST', payload:{name:currentWordList.name, list:filteredCurrentList}})
